...
select * from(
select top 3 * from #bottomstates order by #bottomstates.bottomstate desc) b;
...
(select c.district,c.state state,round(c.population/(c.sex_ratio+1),0) males, round(c.population/(c.sex_ratio+1),0) females from
...
output top 3 districts fom each state with highest literacy rate
